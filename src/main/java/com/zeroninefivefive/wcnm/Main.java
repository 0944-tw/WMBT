...
public final class Main extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        this.getLifecycleManager().registerEventHandler(LifecycleEvents.COMMANDS, command -> {
            LiteralArgumentBuilder<CommandSourceStack> startgame_cmd = Commands.literal("gamestart")
            ...
            LiteralArgumentBuilder<CommandSourceStack> GameJoin = Commands.literal("join")
                    .then(
                            Commands.argument("lobby", StringArgumentType.greedyString())
                                    .executes(ctx -> {
                                        arenaManager.JoinGame((Player) ctx.getSource().getSender(),ctx.getArgument("lobby",String.class));
                                        return Command.SINGLE_SUCCESS;
                                    })
                    );
